...
from emumanager.gui_workers import worker_ps2_convert, worker_dolphin_convert
...
class TestWorkerProgress:
    ...
    def test_ps2_convert_progress(self, mock_find_tool, mock_convert_directory, tmp_path):
        # Setup
        mock_find_tool.return_value = Path("/usr/bin/fake_tool")
        
        # Mock convert_directory to simulate progress calls if we were testing the converter itself,
        # but here we are testing the worker calling the converter with the callback.
    ...
    def test_health_check_progress(self, tmp_path):
        ...
        run_health_check(files, args, base_path, verify_integrity, scan_for_virus, safe_move, logger)
