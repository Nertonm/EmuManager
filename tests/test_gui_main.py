...
@pytest.fixture
def mock_workers():
    with patch("emumanager.gui_main.worker_distribute_root") as mock_dist, \
         ...
         patch("emumanager.gui_main.worker_identify_single_file") as mock_identify_single, \
...
def test_organize_all_button(main_window, mock_workers, tmp_path):
    ...
    # Since we mocked the worker function in `mock_workers` fixture, we can check if it was called.
    
    # However, `on_organize_all` defines an inner function `_work` which calls the worker.
    # So `mock_workers['distribute']` will be called when `_work` runs.
    # We need to ensure `_work` runs.
...
def test_verify_all_button(main_window, mock_workers, tmp_path):
    ...
    if hasattr(main_window.ui, "btn_quick_verify"):
        ...
        with patch.object(main_window.tools_controller, "on_health_check") as mock_health:
...
def test_menu_navigation(main_window, mock_workers, tmp_path):
    ...
    # If mock_workers['tools_cls'] returned a mock, then main_window.tools_controller is that mock instance.
    # And on_organize is a method of that mock.
