...
def test_dolphin_convert_tool_missing(tmp_path, mock_logger, mock_args, mock_converter):
    ...
    with patch(
        "emumanager.workers.dolphin._resolve_dolphin_targets", return_value=[tmp_path]
    ):
        result = worker_dolphin_convert(tmp_path, mock_args, mock_logger, lambda p: [])
...
def test_dolphin_convert_success(tmp_path, mock_logger, mock_args, mock_converter):
    ...
    with patch(
        ...
        mock_converter.convert_to_rvz.assert_called_with(
            iso_file, iso_file.with_suffix(".rvz")
        )
...
def test_dolphin_convert_skip_existing(
    tmp_path, mock_logger, mock_args, mock_converter
):
...
def test_dolphin_convert_rm_originals(tmp_path, mock_logger, mock_args, mock_converter):
    ...
    with (
        patch(
            "emumanager.workers.dolphin._resolve_dolphin_targets",
