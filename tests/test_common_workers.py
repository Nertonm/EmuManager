...
from emumanager.workers.common import calculate_file_hash, create_file_progress_cb, worker_clean_junk, find_target_dir, _clean_junk_files
...
def test_calculate_file_hash(tmp_path):
    ...
    hash_sha1 = calculate_file_hash(f, algo="sha1", chunk_size=10, progress_cb=progress_cb)
...
def test_create_file_progress_cb():
    ...
    file_cb = create_file_progress_cb(main_cb, start_prog=0.0, file_weight=0.5, filename="test.iso")
