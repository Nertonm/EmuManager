...
from emumanager.gui_workers import worker_dolphin_convert, worker_dolphin_verify, worker_dolphin_organize
from emumanager.gui_workers import worker_ps2_organize
...
def mock_converter(monkeypatch):
    ...
    monkeypatch.setattr("emumanager.workers.dolphin.DolphinConverter", MagicMock(return_value=mock))
    return mock
...
def test_worker_dolphin_organize_success(mock_wii_meta, mock_gc_meta, tmp_path, mock_converter):
    # Setup directories
    gc_dir = tmp_path / "roms" / "gamecube"
    gc_dir.mkdir(parents=True)
