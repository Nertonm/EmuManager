...
# Mock GuiLogger to capture logs
class MockLogger:
    def __init__(self):
        self.logs = []
...
def test_clean_junk(tmp_path, mock_args):
    ...
    res = common.worker_clean_junk(tmp_path, mock_args, log_cb, list_files, list_dirs)
...
def test_ps2_verify_no_files(tmp_path, mock_args):
    ...
    # In this test, tmp_path exists but has no subdirs, so find_target_dir might fail or return tmp_path if fallback works?
    # Let's check find_target_dir logic. It checks subdirs. If not found, checks if base_path name matches.
    # tmp_path name is random. So it returns None.
    assert "PS2 ROMs directory not found" in res

@patch("emumanager.ps2.metadata.get_ps2_serial")
@patch("emumanager.ps2.database.db.get_title")
def test_ps2_verify_found(mock_get_title, mock_get_serial, tmp_path, mock_args):
    # Setup
...
def test_psp_verify_no_files(tmp_path, mock_args):
    log_cb = MagicMock()
    res = psp.worker_psp_verify(tmp_path, mock_args, log_cb, lambda p: [])
...
def test_psp_verify_found(mock_get_title, mock_get_meta, tmp_path, mock_args):
    # Setup
    ...
    res = psp.worker_psp_verify(tmp_path, mock_args, log_cb, list_files)
