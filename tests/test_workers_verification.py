...
def test_worker_hash_verify_auto_discovery(tmp_path, mock_logger, mock_args, mock_db):
    ...
    with patch("emumanager.workers.verification.find_dat_for_system", return_value=dat_file), \
         patch("emumanager.workers.verification.dat_parser.parse_dat_file", return_value=mock_db), \
        ...
        mock_logger.assert_any_call(f"Auto-selected DAT: {dat_file.name} (in {dats_root})")
...
def test_worker_hash_verify_parse_error(tmp_path, mock_logger, mock_args):
    ...
    with patch("emumanager.workers.verification.dat_parser.parse_dat_file", side_effect=Exception("Parse Error")):
        report = worker_hash_verify(base_path, mock_args, mock_logger, lambda p: [])
...
def test_worker_identify_all_success(tmp_path, mock_logger, mock_args):
    ...
    # Mock dependencies
    with patch("emumanager.workers.verification.dat_parser.DatDb") as mock_db_cls, \
         patch("emumanager.workers.verification.dat_parser.parse_dat_file") as mock_parse, \
         patch("emumanager.workers.verification._run_verification") as mock_run:
