...
def test_init_connects_signals(mock_main_window):
    ...
    mock_main_window.ui.btn_compress.clicked.connect.assert_called_with(
        controller.on_compress_selected
    )
    mock_main_window.ui.btn_organize.clicked.connect.assert_called_with(
        controller.on_organize
    )
    mock_main_window.ui.btn_dolphin_convert.clicked.connect.assert_called_with(
        controller.on_dolphin_convert
    )
...
def test_on_organize_no_base_dir(tools_controller):
    ...
    with patch("emumanager.controllers.tools.logging") as mock_log:
        ...
        mock_log.warning.assert_called_with(
            "Please select a base directory first (Open Library)."
        )
