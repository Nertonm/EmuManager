...
def test_distribute_root_unknown_system(tmp_path, mock_logger):
    ...
    with patch(
        "emumanager.workers.distributor.guess_system_for_file", return_value=None
    ):
        stats = worker_distribute_root(tmp_path, mock_logger)
...
def test_distribute_root_target_exists(tmp_path, mock_logger):
    ...
    with patch(
        "emumanager.workers.distributor.guess_system_for_file", return_value="nes"
    ):
        stats = worker_distribute_root(tmp_path, mock_logger)
...
def test_distribute_root_cancellation(tmp_path, mock_logger, mock_cancel_event):
    ...
    with patch(
        ...
        stats = worker_distribute_root(
            tmp_path, mock_logger, progress_cb=None, cancel_event=mock_cancel_event
        )
