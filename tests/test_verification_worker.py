...
def test_worker_hash_verify_success(mock_hasher, mock_dat_parser, mock_args, mock_logger, tmp_path):
    # Setup
    ...
    # Mock hasher
    mock_hasher.calculate_hashes.return_value = {"crc32": "12345678", "sha1": "abcdef"}
...
@patch("emumanager.workers.verification.dat_parser")
@patch("emumanager.workers.verification.hasher")
def test_worker_hash_verify_unknown(mock_hasher, mock_dat_parser, mock_args, mock_logger, tmp_path):
    # Setup
    base_path = tmp_path / "roms" / "nes"
    base_path.mkdir(parents=True)
