...
def worker_ps3_verify(base_path: Path, args: Any, log_cb: Callable[[str], None], list_files_fn: Callable[[Path], list[Path]], list_dirs_fn: Callable[[Path], list[Path]] = None) -> str:
...
def _organize_ps3_folders(dirs: list[Path], args: Any, logger: GuiLogger) -> tuple[int, int]:
    renamed = 0
    skipped = 0
    cancel_event = getattr(args, "cancel_event", None)
...
def worker_ps3_organize(base_path: Path, args: Any, log_cb: Callable[[str], None], list_files_fn: Callable[[Path], list[Path]], list_dirs_fn: Callable[[Path], list[Path]] = None) -> str:
