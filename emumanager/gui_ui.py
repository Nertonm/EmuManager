...
class Ui_MainWindow:
    def setupUi(self, MainWindow, qtwidgets: Any):
        ...
        try:
            if _Qt is not None:
                try:
                    MainWindow.addDockWidget(_Qt.DockWidgetArea.BottomDockWidgetArea, self.log_dock)
                except Exception:
    ...
    def _get_icon(self, qt, name: str):
        """Return a QIcon for a given QStyle StandardPixmap name, with safe fallbacks."""
        try:
    ...
    def setupLibraryTab(self, qt, parent):
        ...
        try:
            self.rom_list.setSelectionMode(qt.QAbstractItemView.SelectionMode.ExtendedSelection)
        ...
        self.grp_switch_actions = qt.QGroupBox("Switch Compression Tools (Selected File)")
        ...
        if ic: self.btn_compress.setIcon(ic)
        ic = self._get_icon(qt, "SP_ArrowRight")
        if ic: self.btn_recompress.setIcon(ic)
        ic = self._get_icon(qt, "SP_ArrowUp")
        if ic: self.btn_decompress.setIcon(ic)
    ...
    def _setup_switch_tab(self, qt):
        ...
        try:
            ...
            self.btn_health.setIcon(style.standardIcon(qt.QStyle.StandardPixmap.SP_DialogApplyButton))
            self.btn_switch_compress.setIcon(style.standardIcon(qt.QStyle.StandardPixmap.SP_ArrowDown))
            self.btn_switch_decompress.setIcon(style.standardIcon(qt.QStyle.StandardPixmap.SP_ArrowUp))
        except Exception:
            try:
                style = qt.QApplication.style()
                self.btn_organize.setIcon(style.standardIcon(qt.QStyle.SP_FileDialogListView))
                self.btn_health.setIcon(style.standardIcon(qt.QStyle.SP_DialogApplyButton))
                self.btn_switch_compress.setIcon(style.standardIcon(qt.QStyle.SP_ArrowDown))
    ...
    def _setup_ps2_tab(self, qt):
        ...
        try:
            ...
            self.btn_ps2_verify.setIcon(style.standardIcon(qt.QStyle.StandardPixmap.SP_DialogApplyButton))
            self.btn_ps2_organize.setIcon(style.standardIcon(qt.QStyle.StandardPixmap.SP_FileDialogListView))
        except Exception:
            try:
                style = qt.QApplication.style()
                self.btn_ps2_convert.setIcon(style.standardIcon(qt.QStyle.SP_DriveCDIcon))
                self.btn_ps2_verify.setIcon(style.standardIcon(qt.QStyle.SP_DialogApplyButton))
    ...
    def _setup_ps3_tab(self, qt):
        ...
        try:
            ...
            self.btn_ps3_organize.setIcon(style.standardIcon(qt.QStyle.StandardPixmap.SP_FileDialogListView))
        except Exception:
            try:
                style = qt.QApplication.style()
                self.btn_ps3_verify.setIcon(style.standardIcon(qt.QStyle.SP_DialogApplyButton))
    ...
    def _setup_psp_tab(self, qt):
        ...
        try:
            ...
            self.btn_psp_organize.setIcon(style.standardIcon(qt.QStyle.StandardPixmap.SP_FileDialogListView))
            self.btn_psp_compress.setIcon(style.standardIcon(qt.QStyle.StandardPixmap.SP_ArrowDown))
        except Exception:
            try:
                style = qt.QApplication.style()
                self.btn_psp_verify.setIcon(style.standardIcon(qt.QStyle.SP_DialogApplyButton))
                self.btn_psp_organize.setIcon(style.standardIcon(qt.QStyle.SP_FileDialogListView))
    ...
    def _setup_dolphin_tab(self, qt):
        ...
        try:
            ...
            self.btn_dolphin_convert.setIcon(style.standardIcon(qt.QStyle.StandardPixmap.SP_DriveCDIcon))
            self.btn_dolphin_verify.setIcon(style.standardIcon(qt.QStyle.StandardPixmap.SP_DialogApplyButton))
        except Exception:
            try:
                style = qt.QApplication.style()
                self.btn_dolphin_organize.setIcon(style.standardIcon(qt.QStyle.SP_FileDialogListView))
                self.btn_dolphin_convert.setIcon(style.standardIcon(qt.QStyle.SP_DriveCDIcon))
    ...
    def setup_verification_tab(self, qt, parent):
        ...
        self.btn_verify_dat.setToolTip("Hashes all files in the current library and checks against the selected DAT file.")
        self.btn_verify_dat.setEnabled(False) # Disabled until DAT selected
        
        try:
            style = qt.QApplication.style()
            self.btn_select_dat.setIcon(style.standardIcon(qt.QStyle.StandardPixmap.SP_DialogOpenButton))
            self.btn_verify_dat.setIcon(style.standardIcon(qt.QStyle.StandardPixmap.SP_DialogApplyButton))
        except Exception:
            try:
                style = qt.QApplication.style()
                self.btn_select_dat.setIcon(style.standardIcon(qt.QStyle.SP_DialogOpenButton))
                self.btn_verify_dat.setIcon(style.standardIcon(qt.QStyle.SP_DialogApplyButton))
            except Exception:
                pass
        ...
        self.table_results.setHorizontalHeaderLabels(["Status", "File Name", "Game Name (DAT)", "CRC32", "SHA1", "MD5", "SHA256"])
        try:
        ...
        lbl_dolphin = qt.QLabel("For GameCube and Wii, use the 'Tools' tab -> 'GameCube / Wii' -> 'Identify & Verify Games'.\nThis uses Dolphin's internal verification tool which is recommended for ISO/RVZ/WBFS.")
    ...
    def setup_settings_tab(self, qt, parent):
        ...
        self.chk_process_selected.setToolTip("If checked, operations will only apply to files selected in the Library tab.")
        layout.addRow("Selection:", self.chk_process_selected)

        self.chk_standardize_names = qt.QCheckBox("Enforce Standard Naming")
        self.chk_standardize_names.setToolTip("Rename files to a strict standard (e.g. NoIntro/Redump style) where possible.")
    ...
    def apply_dark_theme(self, qt, qtgui, window):
        try:
            ...
            palette.setColor(qtgui.QPalette.ColorRole.Highlight, qtgui.QColor(50, 150, 250))
            palette.setColor(qtgui.QPalette.ColorRole.HighlightedText, qtgui.QColor(0, 0, 0))
            qt.QApplication.setPalette(palette)
            
            window.setStyleSheet("""
                QMainWindow { background-color: #2d2d2d; }
                QTabWidget::pane { border: 1px solid #3d3d3d; background-color: #2d2d2d; border-radius: 4px; }
                QTabBar::tab { background: #3c3c3c; color: #aaa; padding: 8px 16px; border-top-left-radius: 4px; border-top-right-radius: 4px; margin-right: 2px; font-weight: bold; }
                QTabBar::tab:selected { background: #505050; color: #fff; border-bottom: 2px solid #3daee9; }
                QTabBar::tab:hover { background: #444; color: #ddd; }
                
                QGroupBox { border: 1px solid #444; margin-top: 1.2em; font-weight: bold; border-radius: 6px; padding-top: 12px; padding-bottom: 8px; background-color: #333; }
                QGroupBox::title { subcontrol-origin: margin; subcontrol-position: top left; padding: 0 5px; color: #3daee9; background-color: #2d2d2d; border-radius: 2px; }
                
                QPushButton { padding: 8px 16px; border-radius: 4px; background-color: #3c3c3c; border: 1px solid #555; min-width: 80px; font-weight: bold; }
                ...
                QListWidget { border: 1px solid #444; border-radius: 4px; background-color: #1e1e1e; alternate-background-color: #252525; padding: 4px; }
                QListWidget::item { padding: 4px; border-radius: 2px; }
                QListWidget::item:selected { background-color: #3daee9; color: #fff; }
                QListWidget::item:hover { background-color: #333; }
                
                QTextEdit { border: 1px solid #444; font-family: "Consolas", "Monaco", monospace; background-color: #1e1e1e; border-radius: 4px; padding: 6px; color: #ccc; }
                
                QProgressBar { border: 1px solid #444; border-radius: 4px; text-align: center; background-color: #1e1e1e; color: #fff; font-weight: bold; }
                QProgressBar::chunk { background-color: #3daee9; width: 10px; border-radius: 2px; }
                
                QStatusBar { background-color: #252525; border-top: 1px solid #333; color: #aaa; }
                QLabel { color: #ddd; }
                QSplitter::handle { background-color: #444; width: 2px; }
                
                QComboBox { padding: 4px; border-radius: 4px; background-color: #3c3c3c; border: 1px solid #555; color: #eee; }
                QComboBox::drop-down { border: 0px; }
                QSpinBox { padding: 4px; border-radius: 4px; background-color: #3c3c3c; border: 1px solid #555; color: #eee; }
